...
namespace Lab1
{
    public partial class createService : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                Session["InvalidUse"] = "You must first login to create a new Move.";
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                sqlCommand.Parameters.Add(new SqlParameter("@startDate", startDate.ToString("MM/dd/yyyy HH:mm:ss")));
                sqlCommand.Parameters.Add(new SqlParameter("@completionDate", completionDate.ToString("MM/dd/yyyy HH:mm:ss")));
                ...
                sqlQuery = "INSERT INTO MOVE VALUES("
                    + modified + ", @address, @city, @state, @zip, @daddress, @city, @state, @zip, @notes)";
                ...
                sqlCommand.Parameters.Add(new SqlParameter("@state", ddlState.SelectedItem.ToString()));
                sqlCommand.Parameters.Add(new SqlParameter("@zip", txtZipCode.Text));
                sqlCommand.Parameters.Add(new SqlParameter("@daddress", txtDestinationAddress.Text));
                sqlCommand.Parameters.Add(new SqlParameter("@dcity", txtDestinationCity.Text));
                sqlCommand.Parameters.Add(new SqlParameter("@dstate", ddlDestinationState.SelectedItem.ToString()));
                sqlCommand.Parameters.Add(new SqlParameter("@dzip", txtDestinationZip.Text));
                ...
                outputLbl.Text = HttpUtility.HtmlEncode(CustomerName) + " has a move scheduled on " + HttpUtility.HtmlEncode(startDate.ToString("d"));
            }
        }
        ...
        protected void btnPopulate_Click(object sender, EventArgs e)
        {
            ...
            txtStartDate.Text = "2021-03-22T12:35";
            txtEndDate.Text = "2021-03-22T13:35";
            txtServiceCost.Text = "700";
            txtAddress.Text = "777 Jimmy Street";
            txtCity.Text = "Harrisonburg";
            ddlState.SelectedIndex = 50;
            txtZipCode.Text = "22801";
            txtDestinationAddress.Text = "394 Oak Lane";
            txtDestinationCity.Text = "Harrisonburg";
            ddlDestinationState.SelectedIndex = 50;
            txtDestinationZip.Text = "22801";
            txtNotes.Text = "Provide Employees Lunch";

        }

        // fill dropdowns with states
        ...
        // check that start date is before or the same as end date
        ...
        // check that there is no overlapping services
        protected void checkNotOverlappingService_ServerValidate(object source, ServerValidateEventArgs args)
        {
            DateTime startDate = DateTime.Parse(txtStartDate.Text);
            DateTime completionDate = DateTime.Parse(txtEndDate.Text);
            int customerID = Int32.Parse(ddlCustomer.SelectedValue.ToString());
            String sqlQuery = "Select * from Service WHERE serviceType='M' AND customerID=" + customerID; ;
            // Define the connection to the Database:
            SqlConnection sqlConnect = new SqlConnection(WebConfigurationManager.ConnectionStrings["Connect"].ConnectionString);
            // Create the SQL Command object which will send the query:
            SqlCommand sqlCommand = new SqlCommand();
